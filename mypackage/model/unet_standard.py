...
class NestedUNet_3down(nn.Module):
    #仅有三次下采样，用来选择下采样参数的对比模型
    def __init__(self, in_channels=1, deepsupervision=False, nb_filter=(32, 64, 128, 256),
                 scale_output=True):
        super().__init__()
        self.scale_output = scale_output
        self.deepsupervision = deepsupervision
...
class NestedUNet_2down(nn.Module):
    #仅有三次下采样，用来选择下采样参数的对比模型
    def __init__(self, in_channels=1, deepsupervision=False, nb_filter=(32, 64, 128), scale_output=True):
        super().__init__()
        self.scale_output = scale_output
        self.deepsupervision = deepsupervision
        # nb_filter = [32, 64, 128, 256, 512]
...
class NestedUNet_1down(nn.Module):
    #仅有三次下采样，用来选择下采样参数的对比模型
    def __init__(self, in_channels=1, deepsupervision=False, nb_filter=(32, 64), scale_output=True):
        super().__init__()
        self.scale_output = scale_output
        self.deepsupervision = deepsupervision
        # nb_filter = [32, 64, 128, 256, 512]
...
if __name__ == '__main__':
    ...
    netG = NestedUNet_1down()
    input = torch.zeros((2,1,256,256))
    output = netG(input)
    print(output.shape)
